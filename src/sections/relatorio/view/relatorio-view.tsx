...
export function RelatorioView() {
  ...
  const fetchEmpresaData = useCallback(async () => {
    if (!empresaId) return;
    try {
      const response = await axios.get(`https://fad7-154-71-159-172.ngrok-free.app/api/empresa/${empresaId}/`,{
        headers: {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const response = await axios.get(`https://fad7-154-71-159-172.ngrok-free.app/api/empresa/transacoes/${empresaId}/?page=${page}`,{
        headers: {
          "ngrok-skip-browser-warning": "true", // Evita bloqueios do ngrok
        },
      });
  ...
  const downloadInvoice = async (transacaoId: number) => {
    try {
      const apiUrl = `https://fad7-154-71-159-172.ngrok-free.app/api/fatura/${transacaoId}/`;
          ...
          return (
            <Grid key={report.id} xs={12} sm={6} md={4}>
              <Card>
                <Box
                  component="img"
                  src={`https://fad7-154-71-159-172.ngrok-free.app${report.produto.imagens[0]?.imagem}`}
