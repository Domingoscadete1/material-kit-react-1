...
export function NotificationsPopover({ data = [], sx, ...other }: NotificationsPopoverProps) {
  ...
  useEffect(() => {
    ...
    const fetchNotifications = async () => {
      try {
        const response = await axios.get(`https://fad7-154-71-159-172.ngrok-free.app/api/notificacoes/empresa-list/${empresaId}/`,{
          headers: {
...
function NotificationItem({ notification }: { notification: NotificationItemProps }) {
  ...
  return (
    <ListItemButton sx={{ py: 1.5, px: 2.5, mt: '1px', bgcolor: notification.lida ? 'transparent' : 'action.selected' }}>
      <ListItemAvatar>
        <Avatar src={`https://fad7-154-71-159-172.ngrok-free.app${avatarUrl}`} sx={{ bgcolor: 'background.neutral' }} />
      </ListItemAvatar>
      <ListItemText
        primary={title}
        secondary={
